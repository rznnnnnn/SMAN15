...
// Kelas Section
$pdf->Cell(35, 10, 'nis', 0, 0);
$pdf->Cell(5, 10, ':', 0, 0, 'C');
$pdf->Cell(30, 10, isset($_GET['nis']) ? $_GET['nis'] : 'Default nis', 0, 1); // If 'kelas' is missing, set default

// Retrieve Tahun Ajaran from the database
$tahun_ajaran = '';
if (isset($_GET['nis'])) {
    $nis = $_GET['nis'];
    $result = mysqli_query($konek, "SELECT DISTINCT tahun_ajaran FROM students WHERE nis = '$nis' LIMIT 1");
...
if (isset($_GET['nis'])) {
    ...
    $sql = mysqli_query(
        ...
        WHERE st.nis = '$nis' ORDER BY st.id DESC"
    );
...
$pdf->SetFont('Arial', 'B', 12);
$pdf->Cell(0, 10, 'Tangerang, ' . date('d-m-Y'), 0, 1, 'R');
$pdf->Cell(0, 10, 'Kepala Sekolah SMAN 15 Kota Tangerang,', 0, 1, 'R');
$pdf->Ln(15); // Add space between the date and the signature
